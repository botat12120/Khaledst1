...
const handler = async (m, { conn, args, usedPrefix }) => {
    if (processingRequest) {
        return await m.reply('هناك طلب قيد المعالجة حاليًا. يرجى الانتظار حتى يتم الانتهاء.');
    }
    ...
    try {
        ...
        if (!text.trim()) {
            await m.reply(`*اكتب وصف ما تريد تخيله بالإنجليزية للحصول على نتائج دقيقة.*\n*مثال:*\n*.تخيل* girl with hijab style`);
            return;
        }
